  ...
  try {
    ...
    return res.send(
      response("查询成功", null, {
        ...
        budgetValue: userInfo.budgetValue
      })
    );
...
router.post("/getBudget", async (req, res) => {
  try {
    ...
    const userInfo = await User.findById(query.userId).select(
      "budgetValue budgetPeriod"
    );
    res.send(
      response("查询预算成功", null, {
        value: userInfo.budgetValue,
        period: userInfo.budgetPeriod
      })
    );
  } catch (error) {
    console.log(err);
    return res.send(response("查询预算失败", "query_error"));
  }
});
